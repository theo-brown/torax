...
class IMASGeometryTest(parameterized.TestCase):
  ...
  def test_trapped_fraction_file_and_exact_agree(self):
    """Tests FILE against EXACT for an IDS that provides both.

    The value stored in `profiles_1d.trapped_fraction` by the equilibrium
    code and the bounce-averaged integral recomputed by TORAX from the 2D
    equilibrium implement the same definition, so they should agree to
    within the numerical resolution of the respective calculations (which
    is much tighter than either's agreement with the Sauter approximation).
    """
