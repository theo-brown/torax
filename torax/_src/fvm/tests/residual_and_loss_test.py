...
"""Tests for the residual operator used by the Jacobian-free Newton solver."""
...
def _build_test_case(num_cells: int = 10):
  ...
  torax_config = model_config.ToraxConfig.from_dict(
      dict(
          ...
          transport=dict(model_name='constant', chi_min=0, chi_i=1.0),
...
def _make_preconditioner(x_vec, num_channels, **kwargs):
  ...
  block = lhs.to_block_tridiagonal()
...
class ResidualWithOperatorTest(parameterized.TestCase):
  ...
  def test_operator_clusters_the_jacobian_spectrum(self):
    ...
    This is the property the Jacobian-free solver relies on: GMRES converges in
    a handful of iterations exactly when the preconditioned spectrum is
    clustered. Note that M is *not* the Jacobian -- the sources depend on the
    state, so J - M is a genuinely large term (mainly d(Qei)/dn_e in the
    temperature rows) -- yet it is a small enough perturbation in the right
    sense for the clustering to hold.
    ...
    apply_inverse = _make_preconditioner(
        x_vec, num_channels, **kwargs
    )
