...
class PowerScalingFormationModelTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(
          ...
          scaling_law='martin',
          power=1e6,
          expected_barrier_fraction=1.0,
      ),
      dict(
          testcase_name='martin_below_threshold',
          scaling_law='martin',
          power=-1e6,
          expected_barrier_fraction=0.0,
      ),
      dict(
  ...
  def test_power_scaling_formation_model_barrier_fraction(
      self, scaling_law, power, expected_barrier_fraction
  ):
    ...
    barrier_fraction = formation_model(
        self.runtime_params,
        self.initial_state.geometry,
        self.initial_state.core_profiles,
        high_power_profiles,
    ...
    np.testing.assert_allclose(
        barrier_fraction,
        expected_barrier_fraction,
        atol=1e-3,
        err_msg=(
            f'barrier_fraction={barrier_fraction} is not close to the expected'
            f' value of {expected_barrier_fraction} for scaling law'
            f' {scaling_law}.'
        ),
    )
  ...
  def test_formation_model_uses_hysteresis_threshold_in_h_mode(self):
    ...
    with mock.patch.object(
      ...
      with mock.patch.object(scaling_laws, 'calculate_P_LH') as mock_calc_p_lh:
        ...
        barrier_fraction_l_mode = formation_model(
            runtime_params,
            self.initial_state.geometry,
            self.initial_state.core_profiles,
            self.initial_state.core_sources,
        ...
        barrier_fraction_h_mode = formation_model(
            runtime_params,
            self.initial_state.geometry,
            self.initial_state.core_profiles,
            self.initial_state.core_sources,
    ...
    # more likely to be above threshold, meaning the barrier fraction should
    # be larger (more suppression). In L-mode the full P_LH is used.
    self.assertGreater(
        barrier_fraction_h_mode,
        barrier_fraction_l_mode,
    )
