...
``trapped_fraction_source`` (str [default = 'SAUTER'])
  ...
  * ``'EXACT'``
    ...
    equilibrium. Only supported for EQDSK and IMAS geometries. Flux surfaces
    too small for the 2D grid to resolve reliably (typically only surfaces
    very close to the magnetic axis) fall back to the Sauter approximation;
